...
import { Client, GatewayIntentBits, escapeBulletedList } from 'discord.js';
...
if (!process.env.TOKEN) {
    throw new Error('TOKEN (Discord bot token) is required');
}
...
const prefix = "Trung!ai";

const app = express();

const DiscordMaxLength = 1900
...
if (config['shell-chat']) startChat();
...
client.on('messageCreate', async message => {
    if (message.content.startsWith(prefix)) {
        ...
        if (combinedResponse.length > maxLength) {
            ...
            message.channel.send('AI Original: \n' + combinedResponse.slice(0, DiscordMaxLength) + '...')
        }
        ...
        try {
            ...
            if (combinedResponse.length > DiscordMaxLength) {
                message.channel.send('AI Modified: \n' + combinedResponse.slice(0, DiscordMaxLength) + '...');
            } else {
                message.channel.send('AI Modified: \n' + combinedResponse)
            }
            if (cloudai.length > DiscordMaxLength) {
                message.channel.send('Cloud AI: \n' + cloudai.slice(0, DiscordMaxLength) + '...')
            } else {
                message.channel.send('Cloud AI: \n' + cloudai)
            }
        } catch (err) {
...
client.on('messageCreate', async message => {
    if (message.content.startsWith('Trung!wikitrain')) {
        const userMessage = message.content.slice('Trung!wikitrain'.length).trim();
